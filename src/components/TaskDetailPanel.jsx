...
const SendIcon     = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>;
const TrashIcon    = () => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>;
...
const UploadIcon   = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>;
...
const LinkIcon     = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>;
const ExternalIcon = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>;
...
const TaskDetailPanel = ({ task, assignments = [], onClose, isAdmin }) => {
  const { user } = useAuth();
  ...
  const myAssignment = assignments.find(a => a.user_id === user?.id || a.profiles?.id === user?.id);

  useEffect(() => {
    if (!task) return;
    setComments([]); setAttachments([]); setCommentBody('');
    setLinkUrl(''); setLinkTitle(''); setLinkError('');

    setLoadingComments(true);
    getComments(task.id).then(setComments).catch(() => {}).finally(() => setLoadingComments(false));

    setLoadingAttachments(true);
    getAttachments(task.id).then(setAttachments).catch(() => {}).finally(() => setLoadingAttachments(false));
  }, [task?.id]);
  ...
  const handleSaveLink = async (e) => {
    e.preventDefault();
    setLinkError('');
    if (!linkUrl.trim()) return setLinkError('URL is required.');
  ...
  const fileAttachments = attachments.filter(a => a.attachment_type === 'file' || !a.attachment_type);
    ...
    <div className="task-panel">
      <div className="task-panel__header">
        <div className="task-panel__header-left">
          <span className="task-panel__priority-dot" style={{ background: priorityColor }}/>
          <h2 className="task-panel__title">{task.title}</h2>
        </div>
        <button className="task-panel__close" onClick={onClose}><XIcon /></button>
      </div>
      ...
      <div className="task-panel__body">
        ...
        <div className="task-panel__meta">
          <div className="task-panel__meta-item">
            ...
            <span className="task-panel__meta-value" style={{ color: priorityColor }}>{task.priority}</span>
          </div>
          <div className="task-panel__meta-item">
          ...
          <div className="task-panel__meta-item">
            ...
            <span className={`task-panel__status ${task.completed ? 'task-panel__status--done' : ''}`}>
        ...
        {/* My completion (members only) */}
        {!isAdmin && myAssignment && (
          <div className="task-panel__section">
            <h3 className="task-panel__section-title">My Progress</h3>
            <button
              className={`task-panel__my-done ${myAssignment.completed ? 'task-panel__my-done--done' : ''}`}
              onClick={handleToggleMyDone}
            >
              <div className={`task-panel__my-done-check ${myAssignment.completed ? 'task-panel__my-done-check--filled' : ''}`}>
        ...
        {assignments.length > 0 && (
          <div className="task-panel__section">
            <h3 className="task-panel__section-title">Assigned To ({assignments.length})</h3>
              ...
              {assignments.map(a => {
                ...
                return (
                  <div key={a.id} className="task-panel__assignee">
                    <div className="task-panel__assignee-avatar">{name[0].toUpperCase()}</div>
                    ...
                    <span className={`task-panel__assignee-status ${task.completed ? 'task-panel__assignee-status--done' : ''}`}>
        ...
        <div className="task-panel__section">
          ...
          <div className="task-panel__attach-tabs">
            <button
              className={`task-panel__attach-tab ${attachTab === 'file' ? 'task-panel__attach-tab--active' : ''}`}
              onClick={() => setAttachTab('file')}
            >
              <UploadIcon /> File
            </button>
            <button
              className={`task-panel__attach-tab ${attachTab === 'link' ? 'task-panel__attach-tab--active' : ''}`}
          ...
          {attachTab === 'file' && (
            <div
              className={`task-panel__dropzone ${dragOver ? 'task-panel__dropzone--over' : ''} ${uploading ? 'task-panel__dropzone--uploading' : ''}`}
          ...
          {attachTab === 'link' && (
            <form className="task-panel__link-form" onSubmit={handleSaveLink}>
              <input
                className={`task-panel__link-input ${linkError ? 'task-panel__link-input--error' : ''}`}
              ...
              <button type="submit" className="btn-primary" disabled={savingLink} style={{ width: '100%', justifyContent: 'center' }}>
                <LinkIcon />
            ...
            <div className="task-panel__attachments">
              ...
              {fileAttachments.map(att => (
                <div key={att.id} className="task-attachment">
                  <span className="task-attachment__icon">{getFileEmoji(att.mime_type)}</span>
                  <div className="task-attachment__info">
                    <p className="task-attachment__name">{att.file_name}</p>
                    <p className="task-attachment__size">{formatFileSize(att.file_size)}</p>
                  </div>
                  <div className="task-attachment__actions">
                    <a href={att.url} target="_blank" rel="noopener noreferrer"
                      className="task-attachment__btn task-attachment__btn--download"
                      download={att.file_name} title="Download">
                      <DownloadIcon />
                    </a>
                    {(isAdmin || att.user_id === user?.id) && (
                      <button className="task-attachment__btn task-attachment__btn--delete"
              ...
              {linkAttachments.map(att => (
                <div key={att.id} className="task-attachment task-attachment--link">
                  ...
                  <div className="task-attachment__info">
                    <p className="task-attachment__name">{att.link_title || att.link_url}</p>
                    <p className="task-attachment__size task-attachment__url">{att.link_url}</p>
                  </div>
                  <div className="task-attachment__actions">
                    <a href={att.link_url} target="_blank" rel="noopener noreferrer"
                      className="task-attachment__btn task-attachment__btn--download" title="Open link">
                      <ExternalIcon />
                    </a>
                    {(isAdmin || att.user_id === user?.id) && (
                      <button className="task-attachment__btn task-attachment__btn--delete"
        ...
        <div className="task-panel__section">
                  ...
                  <div key={c.id} className="comment">
                    ...
                    <div className="comment__body">
                      <div className="comment__header">
                        ...
                        {(isOwn || isAdmin) && (
                          <button className="comment__delete" onClick={() => handleDeleteComment(c.id)}>
                            <TrashIcon />
                          </button>
                        )}
                      </div>
          ...
          <form className="task-panel__comment-form" onSubmit={handlePostComment}>
            ...
            <input className="task-panel__comment-input" placeholder="Add a comment..."
              value={commentBody} onChange={e => setCommentBody(e.target.value)} />
            <button type="submit" className="task-panel__comment-send"
