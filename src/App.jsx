...
import {
  ...
  getUsers, getAssignments, assignUsers, unassignUser,
} from './services/index';
...
const AppShell = () => {
  ...
  // Set the email header immediately — must happen before any fetch
  ...
  // ── Theme ──────────────────────────────────────────────────────────────────
  const [themeMode, setThemeMode] = useState(() => localStorage.getItem('tf-theme') ?? 'system');
  ...
  const handleOpenEdit   = (task) => { setTaskToEdit(task); setModalOpen(true); };
  const handleSaveEdit   = async (id, updates) => {
    const updated = await updateTask(id, updates);
    setTasks(prev => prev.map(t => t.id === id ? updated : t));
    toast.success('Task updated!');
  };
  const handleOpenAdd    = () => { setTaskToEdit(null); setModalOpen(true); };
  const handleCloseModal = () => { setModalOpen(false); setTaskToEdit(null); };

  // ── Assignment handlers ────────────────────────────────────────────────────
  const handleOpenAssign  = (task) => { setTaskToAssign(task); setAssignModal(true); };
  const handleCloseAssign = () => { setAssignModal(false); setTaskToAssign(null); };
  ...
  const handleUnassign = async (taskId, userId) => {
    ...
    setAssignments(prev => prev.filter(a => !(a.task_id === taskId && a.user_id === userId)));
  };

  // ── Category handlers ──────────────────────────────────────────────────────
  const handleOpenAddCategory  = () => { setCategoryToEdit(null); setCatModalOpen(true); };
  const handleOpenEditCategory = (cat) => { setCategoryToEdit(cat); setCatModalOpen(true); };
  const handleCloseCatModal    = () => { setCatModalOpen(false); setCategoryToEdit(null); };
  ...
  const myAssignments = assignments.filter(a => a.user_id === user?.id || a.profiles?.id === user?.id);
  const myTaskIds     = new Set(myAssignments.map(a => a.task_id));
  const visibleTasks  = isAdmin ? tasks : tasks.filter(t => myTaskIds.has(t.id));

  const pageProps = {
    tasks: visibleTasks, loading, categories, firstName,
    isAdmin, assignments, users,
  ...
  if (networkError) return (
    <div className="app">
      ...
      <main className="main-content"><NetworkError onRetry={fetchAll} /></main>
    </div>
  );
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case 'tasks':      return <MyTasks    {...pageProps} />;
      case 'calendar':   return <Calendar   {...pageProps} />;
      case 'categories': return <Categories {...pageProps} />;
      case 'settings':   return <Settings   {...pageProps} darkMode={isDark} themeMode={themeMode} onThemeChange={setThemeMode} />;
  ...
  return (
    <div className="app" style={{ position: 'relative' }}>
      <Sidebar activePage={activePage} onNavigate={setActivePage} onSignOut={signOut} />
      <main className="main-content" style={{ marginRight: detailTask ? 400 : 0, transition: 'margin 0.2s ease' }}>
        {renderPage()}
      </main>
      ...
      <AddTaskModal
        ...
        onAddTask={handleAddTask} onSaveEdit={handleSaveEdit}
        categories={categories} taskToEdit={taskToEdit}
      />
      <AddCategoryModal
        isOpen={catModalOpen} onClose={handleCloseCatModal}
        onAddCategory={handleAddCategory} onSaveEdit={handleSaveEditCategory}
        categoryToEdit={categoryToEdit}
      />
      <AssignTaskModal
        isOpen={assignModal} onClose={handleCloseAssign}
        task={taskToAssign}
