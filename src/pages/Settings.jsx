...
const Toggle = ({ checked, onChange }) => (
  <button className={`settings-toggle ${checked ? 'settings-toggle--on' : ''}`}
...
const Settings = ({ themeMode = 'system', darkMode = false, onThemeChange }) => {
  ...
  // ── Save profile — actually updates Supabase auth ─────────────────────────
    ...
    <div className="settings">
      ...
      <Section icon={<ProfileIcon />} title="Profile" subtitle="Update your personal information">
        ...
        <div className="settings-field">
          ...
          <input className="settings-field__input" type="email" value={email}
      ...
      {/* Appearance */}
      <Section icon={<AppearanceIcon />} title="Appearance" subtitle="Customize the look and feel"
      ...
      {/* Notifications */}
      <Section icon={<BellIcon />} title="Notifications" subtitle="Manage how you receive notifications"
        iconColor="#22c55e" iconBg="#f0fdf4">
        {[
          { label: 'Push Notifications',  sub: 'Receive notifications in your browser', val: pushNotifs,  set: setPushNotifs },
          { label: 'Email Notifications', sub: 'Receive task reminders via email',       val: emailNotifs, set: setEmailNotifs },
          { label: 'Sound Effects',       sub: 'Play sounds for notifications',          val: soundFx,     set: setSoundFx },
      ...
      <Section icon={<DatabaseIcon />} title="Data Management" subtitle="Export, import or clear your data">
        <div className="settings-data-btns">
          ...
          <button className="settings-data-btn">Import Data</button>
          <button className="settings-data-btn settings-data-btn--danger"
            onClick={() => { if (window.confirm('Are you sure? This cannot be undone.')) { /* hook up to API */ } }}>
            Clear All Data
          </button>
        </div>
      </Section>
