...
const WEEKLY_DATA = [
  { day: 'Mon', val: 7 }, { day: 'Tue', val: 5 }, { day: 'Wed', val: 2 },
  { day: 'Thu', val: 2 }, { day: 'Fri', val: 4 }, { day: 'Sat', val: 9 }, { day: 'Sun', val: 2 },
];
...
/**
 * Props:
 *   tasks, loading, firstName, isAdmin
 *   users, assignments
 *   onOpenAddTask, onToggle, onEdit, onDelete, onAssign, onClickTask
 */
const Dashboard = ({
  tasks = [], loading, firstName = 'there',
  isAdmin = false, users = [], assignments = [],
  onOpenAddTask, onToggle, onEdit, onDelete, onAssign, onClickTask,
}) => {
  ...
  // ── Per-person progress (admin only) ──────────────────────────────────────
  const memberProgress = isAdmin ? users
  .filter(u => u.email !== import.meta.env.VITE_ADMIN_EMAIL)
  .map(u => {
    const mine = assignments.filter(a =>
      a.user_id === u.id || a.profiles?.id === u.id
    );
    ...
    const pctMine = mine.length > 0
      ? Math.round((doneMine / mine.length) * 100)
      : 0;

    return { ...u, total: mine.length, done: doneMine, pct: pctMine };
  }) : [];

  // ── Per-task progress (admin only) ────────────────────────────────────────
  const taskProgress = isAdmin ? tasks.slice(0, 5).map(task => {
    const taskAssigns = assignments.filter(a => a.task_id === task.id);
    const doneCount   = taskAssigns.filter(a => a.completed).length;
    const pctTask     = taskAssigns.length > 0 ? Math.round((doneCount / taskAssigns.length) * 100) : 0;
    return { ...task, assignedCount: taskAssigns.length, doneCount, pctTask };
  }) : [];

  if (loading) return (
    <div className="dashboard">
      <div className="state-box"><div className="state-box__spinner"/><p>Loading...</p></div>
    </div>
  );
  ...
  return (
    <div className="dashboard">
        ...
        {[
          ...
          <div className="stat-card" key={s.label}>
            <div className="stat-card__icon" style={{ background: s.bg }}>
              <svg viewBox="0 0 24 24" fill="none" strokeLinecap="round" strokeWidth="2"
                style={{ stroke: s.color, width: 20, height: 20 }}>
                <rect x="3" y="3" width="18" height="18" rx="3"/>
              </svg>
            </div>
            <p className="stat-card__value" style={{ color: s.color }}>{s.val}</p>
      ...
      <div className="dashboard__charts">
        ...
        <div className="chart-card">
          ...
          <div className="donut-wrap">
            ...
            <ul className="donut-legend">
              <li className="donut-legend__item"><span className="donut-legend__dot" style={{ background: '#22c55e' }}/>Completed ({completed})</li>
              <li className="donut-legend__item"><span className="donut-legend__dot" style={{ background: '#f59e0b' }}/>Pending ({pending})</li>
            </ul>
          </div>
        </div>
        ...
        <div className="chart-card">
          ...
          <div className="bar-chart-wrap">
            <div className="bar-chart__y-axis">{[12,9,6,3,0].map(n => <span key={n}>{n}</span>)}</div>
            <div className="bar-chart">
              {WEEKLY_DATA.map(d => (
                <div className="bar-chart__col" key={d.day}>
                  <div className="bar-chart__bar" style={{ height: `${(d.val / 12) * 100}%` }}/>
      ...
      {/* ── Admin-only: Per-person progress ─────────────────────────────────── */}
      {isAdmin && memberProgress.length > 0 && (
        <div className="dashboard__section">
          <h3 className="section-header__title" style={{ marginBottom: 12 }}>Team Progress</h3>
              ...
              <div className="dashboard__member-card" key={m.id}>
                <div className="dashboard__member-header">
                  ...
                  <div>
                    ...
                    <p className="dashboard__member-sub">{m.done}/{m.total} tasks</p>
                  </div>
                  <span className="dashboard__member-pct">{m.pct}%</span>
                </div>
                <div className="dashboard__member-track">
                  <div className="dashboard__member-fill" style={{ width: `${m.pct}%` }}/>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── Admin-only: Per-task progress ───────────────────────────────────── */}
      {isAdmin && taskProgress.length > 0 && (
        <div className="dashboard__section">
          <h3 className="section-header__title" style={{ marginBottom: 12 }}>Task Progress</h3>
          <div className="chart-card">
            {taskProgress.map(t => (
              <div className="dashboard__task-progress" key={t.id}>
                ...
                <div className="dashboard__task-progress-bar-wrap">
                  <div className="dashboard__task-progress-bar">
                    <div className="dashboard__task-progress-fill" style={{ width: `${t.pctTask}%` }}/>
                  </div>
      ...
      {overdueTasks.length > 0 && (
        <div className="dashboard__section">
          ...
          <TaskList
            ...
            onAssign={onAssign} onClickTask={onClickTask}
            isAdmin={isAdmin} assignments={assignments}
            empty=""
          />
        </div>
      )}
      ...
      <div className="section-header">
        ...
        {isAdmin && (
          <button className="btn-primary" onClick={onOpenAddTask}><PlusIcon />Add Task</button>
        )}
      </div>
      <TaskList
        tasks={todayTasks}
